...
const index = ({}) => {
  const [bigShowImg, setBigShowImg] = React.useState<string>(bigShoe1);
  return (
    <section
      id='home'
      ...
      <div className='relative flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-hero bg-cover bg-center'>
        <img 
          ...
          <div className='flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:left-[10%] max-sm:px-6'>
            {shoes.map((shoe: {thumbnail:string,bigShoe:string},index)=> (
              <div key={index}>
                <ShoeCard imgURL={shoe} changeBigShoeImage={(shoe)=> setBigShowImg(shoe)} bigShoeImg={bigShowImg}/>
              </div>
            ))}
          </div>
      </div>
