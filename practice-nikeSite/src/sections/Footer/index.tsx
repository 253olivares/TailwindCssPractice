      ...
      <div className='flex justify-between items-start gap-20 flex-wrap max-lg:flex-col'>
        ...
        <div className='flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap'>
            {footerLinks.map((sections,index)=> (
              <div key={index}>
                <h4 className='text-white font-montserrat text-2xl leading-normal font-medium mb-6'>{sections.title}</h4>
                <ul>
                  {sections.links.map((link,index)=> (
                      <li key={index} className='mt3 text-white-400 font-montserrat text-base leading-normal hover:text-slate-gray cursor-pointer'>
                        <a>{link.name}</a>
                      </li>
                  ))}
                </ul>
